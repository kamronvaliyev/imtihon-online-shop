  ...
  async function handleSubmit(evt) {
    ...
    try {
      const res = await axios.put(
        `https://api.escuelajs.co/api/v1/users/${info.id}`,
        {
                ...
                <div className="section__layout">
                  <div className="section__user">
                    {info && (
                      <div className="section__user--box">
                        ...
                        <div className="section__user--info">
                          <h3 className="section__user--name">
                            Name: {info.name}
                          </h3>
                          <p className="section__user--auth">
                            Email: {info.email}
                          </p>
                          <p className="section__user--auth">
                    ...
                    <div className="section__user--logout">
                      <button className="section__btn" onClick={() => logOut()}>
                        Logout
                      </button>
                    </div>
                  </div>
                  <form onSubmit={handleSubmit} ref={ref}>
                    ...
                    <div className="section__btn--boxes">
                      <button
                        type="submit"
                        className="section__btn--save"
                        onClick={() => setSaveLoading(true)}
                      >
                        {saveLoading ? "Loading..." : "Save"}
                      </button>
                      <button
                        className="section__btn--back"
                        onClick={() => navigate("/")}
                      >
                        Back to Home
                      </button>
